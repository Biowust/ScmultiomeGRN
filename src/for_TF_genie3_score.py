...
def GENIE3(expr_data, gene_names=None, regulators='all', tree_method='RF', K='sqrt', ntrees=1000, nthreads=1):
    ...
    if nthreads > 1:
        ...
        for i in tqdm(range(ngenes), desc="Genie3"):
...
def GENIE3_single(expr_data, output_idx, input_idx, tree_method, K, ntrees):
    ...
    if tree_method == 'RF':
        treeEstimator = RandomForestRegressor(n_estimators=ntrees, max_features=max_features, n_jobs=8)
    elif tree_method == 'ET':
        treeEstimator = ExtraTreesRegressor(n_estimators=ntrees, max_features=max_features, n_jobs=8)
...
if __name__=="__main__":
    ...
    output = pd.DataFrame(res, columns=gene_names, index=gene_names)
    # np.savetxt(args.output_file, res, delimiter='\t', header=gene_names)
    output.to_csv(args.output_file, sep='\t')